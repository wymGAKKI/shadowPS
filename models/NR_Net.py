...
class Regressor(nn.Module):
    def __init__(self, batchNorm=False, other={}): 
        ...
        self.est_img = self._make_output(64, 3, k=3, stride=1, pad=1)
        # self.other   = other
    ...
    def forward(self, x, shape):
        ...
        out_img = self.est_img(out)
        out_img = torch.nn.functional.normalize(out_img, 2, 1)
        # normal: torch.Size([1, 3, 172, 172])
        return out_img

class NR_Net(nn.Module):
    def __init__(self, fuse_type='max', batchNorm=False, c_in=3, other={}):
        super(NR_Net, self).__init__()
